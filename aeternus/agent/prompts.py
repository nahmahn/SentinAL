...
from aeternus.dom.views import NodeType, SimplifiedNode
from aeternus.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from aeternus.observability import observe_debug
from aeternus.utils import is_new_tab_page, sanitize_surrogates

if TYPE_CHECKING:
	from aeternus.agent.views import AgentStepInfo
...
class SystemPrompt:
	def __init__(
		...
		is_aeternus_model: bool = False,
	):
		self.max_actions_per_step = max_actions_per_step
		self.use_thinking = use_thinking
		self.flash_mode = flash_mode
		self.is_anthropic = is_anthropic
		self.is_aeternus_model = is_aeternus_model
		prompt = ''
	...
	def _load_prompt_template(self) -> None:
		...
		try:
			...
			if self.is_aeternus_model:
				if self.flash_mode:
					template_filename = 'system_prompt_aeternus_flash.md'
				elif self.use_thinking:
					template_filename = 'system_prompt_aeternus.md'
				else:
					template_filename = 'system_prompt_aeternus_no_thinking.md'
