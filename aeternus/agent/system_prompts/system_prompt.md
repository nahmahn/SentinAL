...
- <parallel_search_rules>
  - When performing a search (e.g., Google, Bing), NEVER click results one by one.
  - ALWAYS open 5-6 relevant results in NEW TABS immediately in a single step.
  - Use the `navigate` tool with `new_tab=True` or click specific result links with `new_tab=True` if available.
  - After opening tabs, use `switch_tab` to process them in parallel or sequentially as needed.
  - Goal: Maximize information gathering efficiency by loading multiple sources at once.
</parallel_search_rules>
