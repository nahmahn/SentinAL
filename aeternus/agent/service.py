...
if TYPE_CHECKING:
	from aeternus.skills.views import Skill

from dotenv import load_dotenv

from aeternus.agent.cloud_events import (
...
from aeternus.agent.message_manager.utils import save_conversation
from aeternus.llm.base import BaseChatModel
from aeternus.llm.exceptions import ModelProviderError, ModelRateLimitError
from aeternus.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from aeternus.tokens.service import TokenCost
...
from aeternus import Browser, BrowserProfile, BrowserSession
from aeternus.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from aeternus.agent.gif import create_history_gif
from aeternus.agent.message_manager.service import (
	MessageManager,
)
from aeternus.agent.prompts import SystemPrompt
from aeternus.agent.views import (
	ActionResult,
	AgentError,
	AgentHistory,
	AgentHistoryList,
...
from aeternus.browser.session import DEFAULT_BROWSER_PROFILE
from aeternus.browser.views import BrowserStateSummary
from aeternus.config import CONFIG
from aeternus.dom.views import DOMInteractedElement
from aeternus.filesystem.file_system import FileSystem
from aeternus.observability import observe, observe_debug
from aeternus.telemetry.service import ProductTelemetry
from aeternus.telemetry.views import AgentTelemetryEvent
from aeternus.tools.registry.views import ActionModel
from aeternus.tools.service import Tools
from aeternus.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_aeternus_version,
	get_aeternus_version,
	time_execution_async,
	time_execution_sync,
)

from aeternus.knowledge.ingestion import ContentIngestor
from aeternus.knowledge.embeddings import MockEmbeddings
from aeternus.verification.service import VerificationService
from aeternus.agent.intent import IntentClassifier, IntentType
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from aeternus.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from aeternus import ChatBrowserUse
		...
		if skill_ids:
			from aeternus.skills import SkillService
		...
		self.agent_directory = base_tmp / f'aeternus_agent_{self.id}_{timestamp}'

		# Initialize file system and screenshot service
		self._set_file_system(file_system_path)
		self._set_screenshot_service()

		# Initialize AI Browser Components
		self.content_ingestor = ContentIngestor()
		...
		self._set_aeternus_version_and_source(source)
		...
		is_aeternus_model = 'browser-use/' in self.llm.model.lower()
		...
		self._message_manager = MessageManager(
			...
			system_message=SystemPrompt(
				...
				is_aeternus_model=is_aeternus_model,
			).get_system_message(),
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'aeternus.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from aeternus.screenshots.service import ScreenshotService
	...
	def _set_aeternus_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the browser-use package"""
		# Use the helper function for version detection
		version = get_aeternus_version()
	...
	async def _prepare_context(self, step_info: AgentStepInfo | None = None) -> BrowserStateSummary:
		...
		# ─── Finance DOM Reader: extract financial fields on financial pages ───
		try:
			from aeternus.dom.finance_dom_reader import FinanceDOMReader
			if browser_state_summary.url and FinanceDOMReader.is_financial_url(browser_state_summary.url):
				finance_summary = FinanceDOMReader.extract_financial_summary(
					dom_state=browser_state_summary.dom_state,
					url=browser_state_summary.url,
				)
				finance_context_text = finance_summary.to_prompt_context()
				if finance_context_text:
					if page_filtered_actions:
						page_filtered_actions = f"{page_filtered_actions}\n\n{finance_context_text}"
					else:
						page_filtered_actions = finance_context_text
					self.logger.info(f'💳 Finance DOM: {len(finance_summary.fields)} financial fields detected')
		except Exception as e:
			self.logger.debug(f'Finance DOM reader skipped: {e}')
	...
	async def _post_process(self) -> None:
		...
		if current_url and current_url != 'about:blank':
			verification_result = self.verification_service.verify_source(current_url)
			if verification_result['status'] == 'flagged':
				self.logger.warning(f"⚠️ Security Alert: {verification_result['reason']}")
			
			# 2. Content Ingestion (only if research intent or general)
			if self.current_intent in [IntentType.RESEARCH, IntentType.GENERAL, IntentType.PLANNING]:
				ingested = await self.content_ingestor.ingest_page(self.browser_session)
				if ingested:
					# Potentially store in memory or knowledge graph
					pass
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from aeternus.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		if CONFIG.BROWSER_USE_VERSION_CHECK:
			latest_version = await check_latest_aeternus_version()
	...
	async def run(
		...
		from aeternus.utils import SignalHandler
		...
		try:
			...
			intent_analysis = await self.intent_classifier.classify(self.task)
			self.current_intent = intent_analysis.intent
			self.logger.info(f"🎯 Intent classified as: {self.current_intent.value.upper()} (Confidence: {intent_analysis.confidence})")
			if intent_analysis.reasoning:
				self.logger.debug(f"   Reasoning: {intent_analysis.reasoning}")
			
			# Update system prompt or context based on intent? (Optional for now)
	...
	async def multi_act(self, actions: list[ActionModel]) -> list[ActionResult]:
		...
		for i, action in enumerate(actions):
			...
			try:
				...
				self.logger.error(f'❌ Executing action {i + 1} failed: {e}')
				raise e
	...
	async def _generate_rerun_summary(
		...
		from aeternus.agent.views import RerunSummaryAction
		...
		from aeternus.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
	...
	async def _execute_ai_step(
		...
		"""
		from aeternus.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from aeternus.llm.messages import SystemMessage, UserMessage
		from aeternus.utils import sanitize_surrogates
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from aeternus.agent.variable_detector import detect_variables_in_history
