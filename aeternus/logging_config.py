...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	...
	if logging.getLogger().hasHandlers() and not force_setup:
		return logging.getLogger('aeternus')
	...
	class BrowserUseFormatter(logging.Formatter):
		...
		def format(self, record):
			...
			if self.log_level > logging.DEBUG and isinstance(record.name, str) and record.name.startswith('aeternus.'):
				...
				if 'Agent' in record.name:
					...
					record.name = 'dom'
				elif record.name.startswith('aeternus.'):
					# For other aeternus modules, use the last part
	...
	# Cosmic Black Theme Configuration
	...
	cosmic_theme = Theme({
		...
		"debug": "dim purple",
		"success": "green bold",
		"agent": "bold #8A2BE2", # Deep Purple
		"browser": "bold #00BFFF", # Deep Sky Blue
		"result": "bold #FFD700", # Gold
	})
	
	rich_console = Console(theme=cosmic_theme)
	...
	# Configure aeternus logger
	aeternus_logger = logging.getLogger('aeternus')
	aeternus_logger.propagate = False  # Don't propagate to root logger
	aeternus_logger.addHandler(console)
	for handler in file_handlers:
		aeternus_logger.addHandler(handler)
	aeternus_logger.setLevel(effective_log_level)
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
	...
	for name in ['aeternus.agent', 'aeternus.tools']:
	...
	for name in ['bubus', 'aeternus.browser.session']:
		logger = logging.getLogger(name)
