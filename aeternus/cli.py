...
if 'init' in sys.argv:
	from aeternus.init_cmd import INIT_TEMPLATES
	from aeternus.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'aeternus_{template}.py'
...
from aeternus import Agent, Controller
from aeternus.agent.views import AgentSettings
from aeternus.browser import BrowserProfile, BrowserSession
from aeternus.logging_config import addLoggingLevel
from aeternus.telemetry import CLITelemetryEvent, ProductTelemetry
from aeternus.utils import get_aeternus_version
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('aeternus.'):
		...
		# Configure aeternus logger and all its sub-loggers
		aeternus_logger = logging.getLogger('aeternus')
		aeternus_logger.propagate = False  # Don't propagate to root logger
		aeternus_logger.handlers = [log_handler]  # Replace any existing handlers
		aeternus_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['aeternus.Agent', 'aeternus.controller', 'aeternus.agent', 'aeternus.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'aeternus.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('aeternus.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('aeternus.app')
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent aeternus from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('aeternus.startup')
	...
	try:
		...
		try:
			from aeternus.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from aeternus.sync.auth import DeviceAuthClient
	...
	try:
		...
		from aeternus.sync.service import CloudSync
		...
		try:
			...
			from aeternus.utils import create_task_with_error_handling
		...
		if task_id and sync_service:
			try:
				from aeternus.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: aeternus_<template>.py)')
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from aeternus.mcp.server import main as mcp_main
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'aeternus_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: aeternus_<template>.py)',
)
@click.option(
	'--force',
	'-f',
